...
class ModelPool():
    ...
    def single_test(self, cnt_round):
        ...
        if 1:
            ...
            df_vehicle_inter_0 = pd.read_csv(os.path.join(path_to_log, "vehicle_inter_0.csv"),
                                             ...
                                             names=["vehicle_id", "enter_time", "leave_time", "sp_vehicle"])

            duration = df_vehicle_inter_0["leave_time"].values - df_vehicle_inter_0["enter_time"].values
            sp_vehicle = df_vehicle_inter_0["sp_vehicle"].values
            com_dur = []
            sp_dur = []
            
            for index in len(duration):
                if sp_vehicle[index]==1 and not isnan(duration[index]):
                    sp_dur.extend(duration[index])
                if sp_vehicle[index]==0 and not isnan(duration[index]):
                    com_dur.extend(duration[index])
                
            
            sp_mean = np.mean(sp_dur)
            com_mean = np.mean(com_dur)
            dur_mean = sp_mean*0.8+com_mean*0.2
            ...
            if nan_num < nan_thres:
                ...
                for i in range(len(self.best_model_pool)):
                    if self.best_model_pool[i][1] > dur_mean:
                        break
                    cnt += 1

                self.best_model_pool.insert(cnt, [cnt_round, dur_mean])
    ...
    def model_compare(self, cnt_round):
        ...
        if 1:
            # summary items (duration) from csv
            # 此处获得vehicle_id车辆在该路口的进入时间和经过时间
            df_vehicle_inter_0 = pd.read_csv(os.path.join(path_to_log, "vehicle_inter_0.csv"),
                                             sep=',', header=0, dtype={0: str, 1: float, 2: float, 3:int},
                                             names=["vehicle_id", "enter_time", "leave_time", "sp_vehicle"])
            ...
            if nan_num < nan_thres:
                ...
                num_max = min(len(self.best_model_pool), self.exp_conf["NUM_BEST_MODEL"])
                self.best_model_pool = self.best_model_pool[:num_max]       #取最后num个
