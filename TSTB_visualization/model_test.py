...
# 20220827 生成log时需要修改以下内容
# TODO test on multiple intersections
def test(model_dir, cnt_round, run_cnt, _dic_traffic_env_conf, records_dir, if_gui):
    dic_traffic_env_conf = deepcopy(_dic_traffic_env_conf)
    # records_dir = model_dir.replace("model", "records")
    model_round = "round_92"  # 1、修改为需要生成log的轮数
    current_round = "round_" + str(cnt_round)
    model_dir = os.path.join("model", "Mylight+GAT_newyork")  # 2、修改为模型存储的文件夹名，存放在model文件夹下
    dic_path = {}
    dic_path["PATH_TO_MODEL"] = model_dir
    dic_path["PATH_TO_WORK_DIRECTORY"] = records_dir

    with open(os.path.join(model_dir, "agent.conf"), "r") as f:
        dic_agent_conf = json.load(f)
    with open(os.path.join(model_dir, "exp.conf"), "r") as f:
        dic_exp_conf = json.load(f)

    if os.path.exists(os.path.join(model_dir, "sumo_env.conf")):
        with open(os.path.join(model_dir, "sumo_env.conf"), "r") as f:
            dic_traffic_env_conf = json.load(f)
    elif os.path.exists(os.path.join(model_dir, "anon_env.conf")):
        with open(os.path.join(model_dir, "anon_env.conf"), "r") as f:
            dic_traffic_env_conf = json.load(f)
    ...
    # dump dic_exp_conf
    with open(os.path.join(model_dir, "test_exp.conf"), "w") as f:
    ...
    try:
        path_to_log = os.path.join(dic_path["PATH_TO_WORK_DIRECTORY"], "test_round", current_round)  # 修改为current_round
        if not os.path.exists(path_to_log):
            os.makedirs(path_to_log)
        #设置环境
        env = DIC_ENVS[dic_traffic_env_conf["SIMULATOR_TYPE"]](path_to_log=path_to_log,
                                                               path_to_work_directory=dic_path[
                                                                   "PATH_TO_MODEL"],  # 修改为path_to_model
        ...
        while not done and step_num < int(dic_exp_conf["RUN_COUNTS"] / dic_traffic_env_conf["MIN_ACTION_TIME"]):
            ...
            for i in range(dic_traffic_env_conf["NUM_AGENTS"]):
                print(dic_traffic_env_conf["NUM_AGENTS"])
                if "CoLight" in dic_exp_conf["MODEL_NAME"] or "MyLight" in dic_exp_conf["MODEL_NAME"]:
                    one_state = state
        ...
        #用于存储各路口每个车辆的到达时间，离开时间，以及sp类型，存储进csv，以及将路口类存入pkl文件
