...
class Updater:
            ...
            try:
                while True:
                    ...
                    if self.dic_exp_conf["PRETRAIN"] or self.dic_exp_conf["AGGREGATE"]:
                        ...
                        #判断sample是否比memory长，长则为遗忘size为len-max_memory
                        ind_sta = max(0, ind_end - self.dic_agent_conf["MAX_MEMORY_LEN"])
                        #截取sample最后的max_memory_len长度的记忆
                        memory_after_forget = cur_round_sample_set[ind_sta: ind_end]
                        # print("memory size after forget:", len(memory_after_forget))

                        # sample the memory， 从记忆中抽取sample进行训练
    ...
    def load_sample_for_agents(self):
        ...
        #非GCN,Colight模型
        if self.dic_exp_conf['MODEL_NAME'] not in ["GCN","CoLight","MyLight"]:
            #One model 或者SimpleDQNOne
            if self.dic_traffic_env_conf["ONE_MODEL"] or self.dic_exp_conf['MODEL_NAME'] in ["SimpleDQNOne"]: # for one model
                ...
                for i in range(self.dic_traffic_env_conf['NUM_INTERSECTIONS']):
                    #不添加遗忘机制，且没有sample_set_all
                    sample_set = self.load_sample(i)
                    #每个agent单独积累自己的sample并进行训练？？
            ...
            if False: # Todo decide multi-process
                for i in range(self.dic_traffic_env_conf['NUM_INTERSECTIONS']):
                    ...
                    if samples_gcn_df is None:
                        ...
                        #pd.merge()对两个pd进行拼接，其中how=inner表示取交集，how=outer表示取并集，缺失值会添加null
                        #on: 要加入的列或索引级别名称。suffixes: 用于重叠列的字符串后缀元组。 默认为（‘x’，’y’）。
    ...
    def update_network(self,i):
        ...
        #下面这一条进行对于网络参数的更新，之后的判断都是用于保存网络模型
        self.agents[i].train_network(self.dic_exp_conf)
        print("train_network complete")
